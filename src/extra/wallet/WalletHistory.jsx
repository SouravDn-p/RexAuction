...
import { useState, useEffect, useRef, useContext } from "react"
import { Calendar, ArrowDown, ArrowUp, ChevronDown, Filter } from "lucide-react"
import { HiOutlinePlus, HiOutlineDocumentDownload } from "react-icons/hi"
import { Link } from "react-router-dom"
import ThemeContext from "../../component/Context/ThemeContext"
import useAuth from "../../hooks/useAuth"
  ...
  const exportToPDF = async () => {
    try {
      ...
      if (transactions.length === 0) {
        doc.text("No transactions found matching your filters", 105, yPos + 10, null, null, "center")
      } else {
        transactions.forEach((transaction, index) => {
          ...
          doc.text(transaction.status === "completed" ? "Completed" : "Pending", 170, yPos + 5)
    ...
    >
      <div className="w-full max-w-5xl mx-auto">
        ...
        >
          <div
            className={`relative overflow-hidden ${
              isDarkMode ? "bg-purple-900" : "bg-gradient-to-r from-purple-600 via-purple-500 to-fuchsia-500"
            } p-8`}
          >
            {!isDarkMode && (
              <>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center relative z-10">
              <div>
                <h2 className="text-3xl font-bold text-white mb-1">Wallet History</h2>
                <p className="text-purple-100 text-sm">Track your financial activities</p>
              </div>
              <div
                className="mt-4 md:mt-0 text-right bg-white/10 backdrop-blur-sm rounded-lg p-4 border border-white/20
                transform transition-all duration-300 hover:scale-105"
              >
                <p className="text-sm text-purple-100 font-medium">Current Balance</p>
                <p className="text-3xl font-bold text-white">
                  {formatNumber(dbUser?.accountBalance)} <span className="text-lg">Taka</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className={`p-6 ${isDarkMode ? "bg-gray-800 text-gray-200" : "bg-white text-gray-800"}`}>
                ...
                <div className="relative" ref={filterRef}>
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Filter className="h-4 w-4 text-purple-500" />
                      <span>
                        {filterType === "all" ? "All Types" : filterType === "deposit" ? "Deposits" : "Withdrawals"}
                      </span>
                    </div>
                    <ChevronDown
                  ...
                  {isFilterOpen && (
                    <div
                      className={`absolute z-10 mt-1 w-full rounded-lg shadow-lg border 
                      transform transition-all duration-300 origin-top ${
                        isDarkMode ? "bg-gray-700 border-gray-600" : "bg-white border-purple-100"
                      }`}
                    >
                      {[
                        { id: "all", label: "All Types" },
                        ...
                        <div
                          key={type.id}
                          className={`py-2 px-4 cursor-pointer first:rounded-t-lg last:rounded-b-lg transition-colors duration-200 ${
                            isDarkMode ? "hover:bg-gray-600 text-white" : "hover:bg-purple-50 text-gray-700"
                          } ${filterType === type.id ? (isDarkMode ? "bg-gray-600" : "bg-purple-50") : ""}`}
                          onClick={() => {
                            setFilterType(type.id)
                            setIsFilterOpen(false)
                          }}
                      ...
                      filteredTransactions?.map((transaction, index) => (
                        ...
                        >
                          ...
                          <td className="py-4 px-6">
                            ...
                            >
                              {transaction.type === "Deposit" ? <ArrowDown /> : <ArrowUp />}
                              {formatNumber(transaction.amount)} Taka
                            </div>
                          </td>
                          <td className="py-4 px-6 text-right">
            ...
            <div className="mt-4 text-center text-sm text-gray-500">
              Showing {filteredTransactions?.length} of {dbUser?.transactions?.length} transactions
            </div>
          </div>
        </div>
      </div>
