  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (Object.keys(localRecentMessages).length === 0) {
          try {
            const messagesResponse = await axios.get(`http://localhost:5000/recent-messages/${currentUser.email}`, {
              withCredentials: true,
            })

            const recentMessagesMap = messagesResponse.data.reduce((acc, { userEmail, lastMessage }) => {
              acc[userEmail] = lastMessage
              return acc
            }, {})

            setLocalRecentMessages(recentMessagesMap)
            localStorage.setItem("recentMessages", JSON.stringify(recentMessagesMap))
          } catch (error) {
    ...
    if (socketRef.current) {
      socketRef.current.on("receiveMessage", (message) => {
        ...
        const otherUserEmail = message.senderId === currentUserEmail ? message.receiverId : message.senderId
            ...
            return (
              <div
                ...
                className={`p-4 cursor-pointer hover:bg-opacity-10 ${
                  ...
                  unreadMessages[user.email] > 0 ? (isDarkMode ? "bg-gray-700 bg-opacity-50" : "bg-purple-50") : ""
              ...
              >
                <div className="flex items-center">
                  <div className="relative">
                    {user.photo ? (
                      <img
                        src={user.photo || "/placeholder.svg?height=40&width=40&text=User"}
                        alt={user.name || "User"}
                      ...
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center text-lg font-semibold ${
                          ...
                          // Add a subtle border for users with recent messages
                          localRecentMessages[user.email] ? "border-2 border-purple-400" : ""
                        }`}
                      >
                  ...
                  <div className="ml-3 flex-1">
                    <div className="flex justify-between items-center">
                      <p className={`${unreadMessages[user.email] > 0 ? "font-bold" : "font-medium"}`}>
                        {user.name || "No name"}
                      </p>
                      {lastMessage && (
                        <p
