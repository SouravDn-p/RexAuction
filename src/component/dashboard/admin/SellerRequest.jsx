  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const acceptedUsers = data.filter(user => user.becomeSellerStatus === "pending");
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (res.data.success) {
            ...
            const response = await fetch("http://localhost:5000/sellerRequest/pending");
            const data = await response.json();
            setUsers(data);
          } else {
                        ...
                        <div className=" flex items-center justify-center gap-2">
                          ...
                          >
                            <li>
                              <button
                                onClick={() =>
                                  handleRoleChange(user._id, "accepted")
                                }
                                className={`${
                                  isDarkMode
                                    ? "hover:bg-gray-600"
