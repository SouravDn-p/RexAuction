...
export default function UpdateBlog() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.patch(`http://localhost:5000/updateBlog/${id}`, updatedBlog);
    ...
    return (
        <div className={`min-h-screen p-6 md:p-8 ${isDarkMode ? "bg-gray-900 text-white" : "bg-gradient-to-b from-purple-100 via-white to-purple-50 text-gray-800"}`}>
            <div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 p-8 rounded-xl shadow-md relative">
                {/* Back Button */}
                <button onClick={() => navigate(-1)} className="absolute top-4 left-4 flex items-center gap-2 text-purple-600 dark:text-purple-300 hover:text-purple-800 dark:hover:text-purple-400 transition">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value={blogData.title} onChange={handleChange}
                            className={`w-full p-3 rounded-md border ${isDarkMode ? "border-gray-700 bg-gray-700 text-white" : "border-gray-300 bg-white text-gray-800"} focus:outline-none focus:ring-2 transition`}
                            required />
                    </div>
                    ...
                    <div>
                        ...
                        <input type="file" name="image" accept="image/*" onChange={handleChange} multiple
                        ...
                        {newImages.length > 0 && (
                            <p className="text-xs mt-1 text-green-500">Selected {newImages.length} images</p>
                        )}
                    </div>
                    ...
                    {blogData.imageUrls.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                            {blogData.imageUrls.map((url, index) => (
                                <img key={index} src={url} alt="Existing" className="w-16 h-16 object-cover rounded-md" />
                            ))}
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        <textarea name="fullContent" value={blogData.fullContent} onChange={handleChange} rows="6"
                            className={`w-full p-3 rounded-md border ${isDarkMode ? "border-gray-700 bg-gray-700 text-white" : "border-gray-300 bg-white text-gray-800"} focus:outline-none focus:ring-2 transition`}
                            required></textarea>
                    </div>
