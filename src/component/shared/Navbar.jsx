...
const Navbar = () => {
  const { user, setUser, setLoading, setErrorMessage, dbUser, setDbUser, walletBalance, setWalletBalance } = useContext(AuthContexts)
  ...
  const handleGoogleSignOut = async () => {
    ...
    try {
      ...
      setShowProfileMenu(false)
      toast.success("Successfully signed out", { position: "top-right", autoClose: 3000 })
    } catch (err) {
  ...
  const handleDepositSubmit = async () => {
    ...
    try {
      const res = await axiosPublic.patch(`/accountBalance/${dbUser._id}`, { accountBalance: updatedBalance })
      if (res.data.success) {
  ...
  const navRoutes = [
    ...
    ...(user?.email ? [{ path: "/auction", label: "Auction", icon: <FaGavel className="w-5 h-5" /> }] : []),
    { path: "/aboutUs", label: "About Us", icon: <FiInfo className="w-5 h-5" /> },
      ...
      >
        <div className="container mx-auto flex justify-between items-center px-4">
            ...
            <div className="flex items-center space-x-6">
              {navRoutes.map((item) => (
                ...
                >
                  ...
                  <span className="relative z-10 transition-transform duration-300 group-hover:transform group-hover:translate-y-[-2px]">{item.label}</span>
                  <span
            ...
            {user?.email ? (
              <>
                ...
                >
                  ...
                  <div className={`absolute inset-0 rounded-full transition-all duration-300 opacity-0 hover:opacity-100 ${isDarkMode ? "bg-yellow-400/10" : "bg-indigo-700/10"}`}></div>
                </button>
                ...
                <div className="relative" ref={profileRef}>
                  ...
                  >
                    <img
                      src={user?.photoURL || "https://i.ibb.co/Y75m1Mk9/Final-Boss.jpg"}
                      alt="Profile"
                  ...
                  {showProfileMenu && (
                    ...
                    >
                      <div className={`px-4 py-3 ${isDarkMode ? "border-b border-indigo-700/50" : "border-b border-indigo-200/50"}`}>
                        <p className={`font-semibold text-sm ${isDarkMode ? "text-white" : "text-gray-800"}`}>{user?.displayName || "User"}</p>
                        <p className={`text-xs truncate ${isDarkMode ? "text-indigo-200" : "text-indigo-600"}`}>{user?.email}</p>
                        <span className="inline-block px-2 py-0.5 mt-1 bg-gradient-to-r from-pink-500 to-purple-500 rounded-full text-white text-xs font-semibold capitalize">{dbUser?.role || "Guest"}</span>
                      </div>

                      {[
                        { to: "/dashboard/profile", icon: <FaUserCircle />, label: "Your Profile" },
                        { to: "/dashboard", icon: <MdOutlineDashboard />, label: "Dashboard" },
                        { to: "/dashboard/walletHistory", icon: <BiMoney />, label: "Wallet History" },
                      ].map((item, index) => (
                        <Link
                          key={index}
                          to={item.to}
                          className={`flex items-center gap-3 py-2 px-4 transition-all duration-200 relative overflow-hidden ${
                            isDarkMode ? "text-indigo-100 hover:bg-indigo-700/70 hover:text-white" : "text-indigo-800 hover:bg-indigo-100 hover:text-indigo-900"
                          }`}
                        >
                          <span className={`${isDarkMode ? "text-indigo-300" : "text-indigo-700"} transition-all duration-300`}>{item.icon}</span>
                      ...
                      <div className={`mt-1 ${isDarkMode ? "border-t border-indigo-700/50" : "border-t border-indigo-200/50"}`}>
                        <button
                          onClick={handleGoogleSignOut}
                          className={`w-full flex items-center gap-3 py-2 px-4 transition-all duration-200 relative overflow-hidden ${
                            isDarkMode ? "text-red-300 hover:bg-red-900/50 hover:text-red-200" : "text-red-600 hover:bg-red-100 hover:text-red-700"
                          }`}
                        >
              ...
              >
                ...
                <span className="relative z-10 transition-transform duration-300 group-hover:transform group-hover:translate-y-[-2px]">Login</span>
                <span
          ...
          <div className="lg:hidden flex items-center space-x-2">
              ...
              <div className="hidden">
                <button
                  className={`flex items-center gap-1 py-1.5 px-3 rounded-lg text-sm transition-all duration-300 relative overflow-hidden ${
                    isDarkMode ? "bg-indigo-700/50 text-white hover:bg-indigo-600/70" : "bg-indigo-200/50 text-indigo-900 hover:bg-indigo-300/70"
                  } hover:scale-105`}
                  onClick={() => setShowWalletModal(true)}
                >
            ...
            <button
              className={`p-2 rounded-full transition-all duration-300 relative overflow-hidden ${
                isDarkMode ? "bg-indigo-800/50 text-yellow-400 hover:bg-indigo-700/70" : "bg-indigo-100/50 text-indigo-700 hover:bg-indigo-200/70"
              } hover:scale-110`}
              onClick={toggleTheme}
              aria-label={isDarkMode ? "Switch to light mode" : "Switch to dark mode"}
            >
            ...
            >
              {mobileMenuOpen ? (
                <svg className="w-6 h-6 relative z-10 transition-transform duration-300 hover:rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
              ) : (
                <svg className="w-6 h-6 relative z-10 transition-transform duration-300 hover:rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div
          ...
          } ${isDarkMode ? "bg-gradient-to-b from-gray-900 to-indigo-900" : "bg-gradient-to-b from-indigo-50 to-purple-100"} rounded-r-xl shadow-2xl`}
        >
          <div className="relative h-full flex flex-col">
            <div className="absolute top-4 right-4 z-10">
              <button
                className={`p-2 rounded-full transition-all duration-200 ${isDarkMode ? "text-indigo-300 hover:bg-indigo-800/50" : "text-indigo-700 hover:bg-indigo-200"}`}
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-500 scrollbar-track-transparent p-5">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h1 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-yellow-400">Rex Auction</h1>
              </div>

              {user?.email && (
                <div
                  className={`rounded-xl p-3 mb-6 shadow-lg ${isDarkMode ? "bg-indigo-800/60 border border-indigo-700/40" : "bg-white/90 border border-indigo-200/40"} transition-all duration-200 hover:shadow-xl`}
                >
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <div>
                      <p className={`font-bold text-sm ${isDarkMode ? "text-white" : "text-gray-800"}`}>{user?.displayName || "User"}</p>
                      <p className={`text-xs ${isDarkMode ? "text-indigo-200" : "text-indigo-600"}`}>
                        <span className="inline-block px-2 py-0.5 bg-gradient-to-r from-pink-500 to-purple-500 rounded-full text-white text-xs font-semibold mt-1 capitalize">{dbUser?.role || "Guest"}</span>
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-1">
                <p className={`text-xs font-semibold uppercase tracking-wider mb-2 pl-2 ${isDarkMode ? "text-indigo-300" : "text-indigo-700"}`}>Navigation</p>
                {navRoutes.map((item) => (
                  <Link key={item.path} to={item.path} className={getNavLinkClass(item.path)} onClick={() => setMobileMenuOpen(false)}>
                ...
                {user?.email && (
                  <>
                    <p className={`text-xs font-semibold uppercase tracking-wider mt-4 mb-2 pl-2 ${isDarkMode ? "text-indigo-300" : "text-indigo-700"}`}>User</p>
                    ...
                    <Link to="/dashboard/profile" className={getNavLinkClass("/dashboard/profile")} onClick={() => setMobileMenuOpen(false)}>
              ...
              <div className={`mt-4 pt-4 border-t ${isDarkMode ? "border-indigo-700/40" : "border-indigo-200/40"}`}>
                {user?.email ? (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 py-2 px-3 rounded-lg transition-all duration-200 ${isDarkMode ? "bg-red-900/50 text-red-300 hover:bg-red-900/70" : "bg-red-50 text-red-600 hover:bg-red-100"}`}
                  >
                    <MdOutlineLogout className={`w-4 h-4 ${isDarkMode ? "text-red-300" : "text-red-600"}`} />
                    <span>Logout</span>
                  </button>
                ) : (
                  <Link to="/login" className={`w-full flex items-center gap-3 py-2 px-3 rounded-lg transition-all duration-200 ${isDarkMode ? "bg-indigo-700/70 text-white hover:bg-indigo-700/90" : "bg-indigo-200 text-indigo-900 hover:bg-indigo-300"}`} onClick={() => setMobileMenuOpen(false)}>
                    <FaUserCircle className={`w-4 h-4 ${isDarkMode ? "text-indigo-300" : "text-indigo-700"}`} />
                    <span>Login</span>
                  </Link>
                )}
        ...
        {mobileMenuOpen && (
          <div className="lg:hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-40 transition-opacity duration-200" onClick={() => setMobileMenuOpen(false)}></div>
        )}
      </nav>
    </div>
  )
